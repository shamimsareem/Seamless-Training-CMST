...
class SCAtt(BasicAtt):
    ...
    def forward(self, att_map, att_mask, value1, value2):
        ...
        if att_mask is not None:
            ...
            att_map_pool = torch.sum(att_map * att_mask_ext, -2) / torch.sum(att_mask_ext, -2)  #equation 4
        else:
        ...
        alpha_spatial = F.softmax(alpha_spatial, dim=-1)  #beta^s = softmax(b^s) equation 3
        ...
        if len(alpha_spatial.shape) == 4: # batch_size * head_num * seq_num * seq_num (for xtransformer)
            ...
            value2 = torch.matmul(alpha_spatial.unsqueeze(-2), value2).squeeze(-2)      #(for XLAN)
